...
- 公告索引优先使用巨潮；巨潮接口拒绝服务时依次回退到东方财富公告库与公司所属交易所的公告接口（深交所 `szse.cn/api/disc`、上交所 `query.sse.com.cn`），正文仍取同一份文件的原始 PDF，`sources` 中的 `publisher` 标明实际提供方。上交所的公告 PDF 主机带 JS 校验，回退到上交所时只提供公告标题与链接，不提供原文抽取。
...
- 主力资金与筹码成本是交易平台的推算口径；东方财富不可用时改用新浪财经全单净流入口径，响应中的 `caliber` 标明实际口径。东方财富历史行情接口按镜像域名重试。
- 筹码成本由本站按东方财富公开的三角形筹码衰减模型自日 K 线推算，与东方财富页面同算法；东方财富历史行情不可用时改用腾讯证券日 K 线，换手率按当前流通股本折算，响应中会给出该差异的警告。
